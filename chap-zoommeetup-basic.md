...
[^Zoomlogin]: https://zoom.us/pricing
...
!["ミーティングの設定ができたよ画面"](chap-zoommeetup-basic/zm07.jpg?scale=0.6)
...
なお、アカウント毎に保存容量が決まっており、Proアカウントで最低限の契約としていた場合、現時点では1GBのクラウドストレージ容量が提供されます。
...
ただクラウドストレージの容量を消費し、そもそものストレージ容量がそこまで大きくない課題もあり、筆者としてはZoomでの勉強会録画の配信は必要最低限にとどめておくのが良さそうな感じを受けています。
（ホストは動画をダウンロードできるので、それをYoutubeに公開なり非公開設定を含めてアップロードするなどのアプローチのほうがコスト的にも運用的にも良い感じになると思われます。）
...
内容を閲覧することはできないのですが、なぜかクラウドストレージの容量は食われてしまうというバグがあるようです。
以前に試した時は4時間程度しか勉強会を行っていないのにもかかわらず、2.5T超の容量を使ってしまっており、びっくりした覚えがあります。

なお、ブレイクアウトルーム内でのそれぞれの内容を録画したい場合は各ルーム内で誰かが録画ボタンを押せるように権限設定し、
更にそのようにオペレーションしてもらう必要があるようです。（あまり必要性が見当たらなかったので私のほうでは未検証です。）
